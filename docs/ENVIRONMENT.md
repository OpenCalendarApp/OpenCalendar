...
| --- | --- | --- | --- |
| `POSTGRES_DB` | `session_scheduler` | No | Database name used by local scripts, connection-string fallback, and Docker Postgres initialization. |
...
| `DOCKER_DATABASE_URL` | `postgresql://ss_admin:change-me@postgres:5432/session_scheduler` (compose fallback) | No | DB connection string injected into the Docker `server` container as `DATABASE_URL`. Use `postgres` as host when containerized. |
...
| `EMAIL_FROM` | `no-reply@session-scheduler.local` | No | Sender address for booking emails. |
...
| `VITE_DEV_PROXY_TARGET` | `http://localhost:4000` | No | Vite dev-server proxy target for `/api` requests. |
